...
lower=8;
upper=38;
arbitraryCutoffs=[lower,lower,lower,lower;upper,upper,upper,upper];
...
if(boolFC)
    strCutoffs="["+strjoin(reshape((string(arbitraryCutoffs)).',1,[]),"_")+"]";
    folderName="nROI="+nROI+"_"+normMethod+"_FC="+methodFC+"_FCtoAdj="+FCtoAdj+"_groupFC="+boolGroupFC+...
        "_capOutliers="+capOutliers+"_cutoff="+cutoff_option+"_cutoffVals="+...
        strCutoffs+"_start="+startInd+"_end="+endInd;
...
% Pre-allocate space
meanLowU=zeros(nROI,nGroups);meanHighU=zeros(nROI,nGroups);
stdLowU=zeros(nROI,nGroups);stdHighU=zeros(nROI,nGroups);
meanLowX=zeros(nROI,nGroups);meanHighX=zeros(nROI,nGroups);
stdLowX=zeros(nROI,nGroups);stdHighX=zeros(nROI,nGroups);

meanEigVecs=zeros(nROI,nROI,nGroups);

% Brain plots
for g=1:nGroups
    % Mean and std magnitude of eigvec over freqs and sessions
...
f2=figure;
for g=1:nGroups
    meanPlot=mean(cMat(:,:,indsPDHC{g}),3);
    varPlot=var(cMat(:,:,indsPDHC{g}),0,3);
    logStr="";
    if(boolLog)
        logStr="Log ";
    ...
    figure(f1);
    subplot(2,2,subplotsInds{g});imagesc(meanPlot);
    title(logStr+"Mean"+cStr+plotLegends(g));
    xlabel('regions');ylabel('regions');colorbar; 
    figure(f2);
    subplot(2,2,subplotsInds{g});imagesc(varPlot);
    title(logStr+"Var"+cStr+plotLegends(g));
    xlabel('regions');ylabel('regions');colorbar; 
end
if(boolSavePlots)
    saveas(f1,savePlotsFolder+"/meanFC.png")
    saveas(f2,savePlotsFolder+"/varFC.png")
end
...
title("Mean "+sgtit);
legend(plotLegends,'Location','best');
