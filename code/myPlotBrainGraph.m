...
% Inspiration from:
% Jonas Richiardi: This function uses routines formerly implemented in MIP:Lab 
% to plot brain plots
%
% Inputs:
...
% - phereSize rescales the size of the nodes; recommanded value is
% the max of the CC vector or something similar
% - Same for SphereCol, but controling the color of the nodes
...
% - ColSph: Colour for each sphere
% - SizSph: Size for each sphere

function [] = myPlotBrainGraph(CM,CC,CC2,CodeBook,T_conn,SphereSize,...
    SphereCol,Exp_Sphere,View,Colormap_nodes,Colormap_edges,Gamma,...
    LinearWeight,CA)
    ...
    % node size and color
    pp.FctForColSph=SphereCol;
    pp.FctForSizSph=SphereSize;

    % Edge transparency
    pp.doAlpha = true;
    pp.alphagamma = 0.05;

    % If we have max and min values for the colormap
    if ~isempty(CA)
