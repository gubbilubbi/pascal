function [corrMeans,corrStds]=create_corr_timeseries(data,connType,indsPDHC,saveFolder)
    ...
    switch connType
        case 'corr'
            for s=1:nSubjs
                subData=data(:,:,s);
                [corrMeans(:,:,s),corrStds(:,:,s)]=create_dot_timeseries(subData);
            end
        case 'Avgcorr'
            for i=1:length(indsPDHC)
                ...
                [groupCorrMeans,groupCorrStds]=create_dot_timeseries(groupROIData);
                for s=indsPDHC{i}
...
function [corrMeans,corrStds]=create_dot_timeseries(data)
    nTimes=size(data,1);
